...
When `chronicle.init.runnable` is set, the named class runs *before* Chronicle reads any system properties. The optional `chronicle.postinit.runnable` class runs once after all static initialisers complete.
