...
Chronicle-Core is an advanced low-level library that equips developers writing low-latency Java code with powerful tools to interact with the operating system, manage memory and handle resources.
Misuse requires caution because many operations bypass the usual JVM safety checks.

Here is a summary of the library's key features:

=== Memory Management
* Off-heap memory and memory-mapped files for deterministic allocation <<memory-management-guide,guide>>.
* Object pooling and class-local caching minimise garbage <<object-pooling-and-caching,guide>>.

=== OS & JVM Interaction
* Helpers to query the operating system and JVM runtime <<jvm-and-os-utilities,guide>>.
* Automatic loading of system properties <<system-properties-from-file,System properties from file>>.

=== Resource Lifecycle
* Deterministic closing and reference counting <<deterministic-resource-management,guide>>.
* Optional background releaser thread for heavy clean-up <<deterministic-resource-management,guide>>.
* Thread-safety checks catch accidental sharing <<thread-safety-guarantees,guide>>.

=== Utilities
* Maths helpers for rounding and range checks <<advanced-maths-functions,Advanced Maths Functions>>.
* Serializable lambdas and a high performance histogram implementation.
...
A number of relevant system properties are listed in <<system-properties,System Properties Overview>>.

NOTE: Command line-specified system properties override those in the `system.properties` file.

== Chronicle-Core Initialization

Chronicle-Core offers an initialisation hook class, link:https://github.com/OpenHFT/Chronicle-Core/blob/ea/src/main/java/net/openhft/chronicle/core/ChronicleInit.java[`ChronicleInit`].
When `chronicle.init.runnable` is set, the named class runs *before* Chronicle reads any system properties.
The optional `chronicle.postinit.runnable` class runs once after all static initialisers complete.

As an alternative, implement the `ChronicleInitRunnable` interface and list the implementation under `META-INF/services/net.openhft.chronicle.core.ChronicleInitRunnable`.
The `run()` method executes at the pre-init stage and `postInit()` after the static initialisation.

`ChronicleInit` introduces the following system properties:
...
Alternatively, implement `ChronicleInitRunnable` and list the class under `META-INF/services/net.openhft.chronicle.core.ChronicleInitRunnable`.
The `run()` method fires during the pre-init phase and `postInit()` after static initialisation.
...
Each check performs a small thread-id comparison, so leave it enabled only if the extra safety outweighs the cost.
...
JLBH has moved home and now lives in its own project, see link:https://github.com/OpenHFT/JLBH[JLBH[https://github.com/OpenHFT/JLBH[JLBH]].
...
* <<memory-management-guide,Memory Management Guide>>
* <<jvm-and-os-utilities,JVM and OS Utility Highlights>>
* <<deterministic-resource-management,Deterministic Resource Management>>
* <<object-pooling-and-caching,Object Pooling and Caching>>
* <<thread-safety-guarantees,Thread Safety Guarantees>>
* <<advanced-maths-functions,Advanced Maths Functions>>
* <<system-properties,System Properties Overview>>
