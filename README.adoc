...
The link:https://github.com/OpenHFT/Chronicle-Core/blob/ea/src/main/java/net/openhft/chronicle/core/ChronicleInit.java[`ChronicInit`] class handles application code which must be loaded/run before any system property is read and may override system properties.
...
The class should contain an empty static init() method that is called to trigger class load. The code should reside in a static block to be run once.
