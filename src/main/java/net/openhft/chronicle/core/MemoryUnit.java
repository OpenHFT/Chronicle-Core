...
public enum MemoryUnit {
    ...
    /**
     * Equivalent of {@code align(amount, unit) == amount}.
     */
    public boolean aligned(long amount, MemoryUnit unit) {
        return align(amount, unit) == amount;
    }

    /**
     * Throws {@code IllegalArgumentException}, if {@code aligned(amount, unit)} is {@code false}.
     */
    public void checkAligned(long amount, MemoryUnit unit) {
        if (!aligned(amount, unit))
            throw new IllegalArgumentException(amount + " " + amount +
                    " is not aligned to " + this);
    }
    ...
    /**
     * Checks the given memory amount in the given unit is an integral number of this units,
     * converts to this unit if so, throws {@code IllegalArgumentException} otherwise.
     *
     * @param sourceAmount the memory amount in the given {@code sourceUnit}
     * @param sourceUnit the unit of the {@code sourceAmount} argument
     * @return the converted amount in this unit
     * @throws IllegalArgumentException if the given {@code sourceUnit} is finer than this unit,
     * or if the given memory unit is not integral in this unit
     */
    public long checkAlignedAndConvert(long sourceAmount, MemoryUnit sourceUnit) {
        checkAligned(sourceAmount, sourceUnit);
        return convert(sourceAmount, sourceUnit);
    }
