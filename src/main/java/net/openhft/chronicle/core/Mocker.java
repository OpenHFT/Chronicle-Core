...
public final class Mocker {
 ...
 using the provided consumer,
    ...
    private static <T> void addInterface(Set<Class<?>> interfaceTypes, Class<T> tClass) {
        if (Jvm.dontChain(tClass))
            return;
        if (interfaceTypes.contains(tClass))
            return;
        interfaceTypes.add(tClass);
        for (Method method : tClass.getMethods()) {
            ...
            if (returnType0 instanceof Class) {
                ...
                if (returnType.isInterface())
                    addInterface(interfaceTypes, returnType);
            }
        }
    }
