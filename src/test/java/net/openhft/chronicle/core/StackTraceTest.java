...
class StackTraceTest extends CoreTestCommon {
    ...
    void testConstructorWithMessageAndCause() {
        ...
        assertTrue(st.getMessage().matches(message + " on " + Thread.currentThread().getName() + " at " + TIMESTAMP_REGEX),
                String.format("%s with cause must match regular expression expecting '%s on %s at' with following timestamp",
    ...
    void testForThread_NullThread() {
        assertNull(StackTrace.forThread(null), "StackTrace.forThread should return null for null thread");
    }
    ...
    void forThread() throws InterruptedException {
        ...
        if (Jvm.isJava20Plus()) {
            ...
            verifyTopFramesContainPauseOrSleep(st);
        } else {
    ...
    void testTimeIsUTC() {
        ...
        assertTrue(msg.endsWith("Z"), msg + " should end with Z for UTC timestamp");
    }

    private static void verifyTopFramesContainPauseOrSleep(StackTrace st) {
