...
class UnsafeMemoryTest extends CoreTestCommon {
    ...
    private static final class UnsafeMemoryParameterResolver implements ParameterResolver {
        ...
        public Object resolveParameter(ParameterContext parameterContext, ExtensionContext extensionContext) {
            ...
            throw new IllegalArgumentException("Unsupported parameter type for test: " + parameterContext.getParameter());
        }
    }

    @BeforeEach
    public void setUp() {
        assertNotNull(caseName, "case name should be set for each test template invocation");
    ...
    void testUnsafeBooleanOperations() throws NoSuchFieldException {
        ...
        assertFalse(testObj.booleanField, "boolean field should default to false");
    ...
    void testUnsafeDoubleOperations() throws NoSuchFieldException {
        SampleClass testObj = new SampleClass();
        assertEquals(0.0, testObj.doubleField, 0.0, "double field should default to 0.0");
    ...
    void testTestAndSetInt() {
        ...
        try {
            ...
            assertThrows(IllegalStateException.class, () -> memory.testAndSetInt(address, 0, 0, 20),
                    "testAndSetInt should throw when value already updated");
        } finally {
            UnsafeMemory.UNSAFE.freeMemory(address);
        }
    }
    ...
    void testTestAndSetIntMemoryAddress() {
        ...
        try {
            ...
            assertThrows(IllegalStateException.class, () -> memory.testAndSetInt(address, 0, expected, 300),
                    "testAndSetInt should throw when expected mismatches");
        } finally {
            UnsafeMemory.UNSAFE.freeMemory(address);
        }
    }
    ...
    void testTestAndSetIntObjectField() throws NoSuchFieldException {
        ...
        assertThrows(IllegalStateException.class, () -> memory.testAndSetInt(obj, offset, expected, 30),
                "testAndSetInt should throw when object value changed");
    }
        ...
        for (int i = 0; i <= 64; i++) {
            if (onHeap == null) {
                ...
                assertEquals((short) 0xABCD, memory.readShort(addr + i),
                        "off-heap writeShort should store value readable at offset " + i);
            } else {
                memory.writeShort(object, addr + i, (short) 0xABCD);
                assertEquals((short) 0xABCD, memory.readShort(object, addr + i),
                        "on-heap writeShort should store value readable at offset " + i);
            }
        }
    }
        ...
        for (int i = 0; i <= 64; i++)
            if (onHeap == null) {
                ...
                assertEquals(INT_VAL, memory.readInt(addr + i),
                        "off-heap int read should return written value at offset " + i);
            } else {
                memory.writeInt(object, addr + i, INT_VAL);
                assertEquals(INT_VAL, memory.readInt(object, addr + i),
                        "on-heap int read should return written value at offset " + i);
            }
    }
    ...
    void writeOrderedInt() {
        for (int i = 0; i <= 64; i++)
            if (onHeap == null) {
                ...
                assertEquals(INT_VAL, memory.readInt(addr + i),
                        "off-heap ordered int write should be visible to read at offset " + i);
            } else {
                memory.writeOrderedInt(object, addr + i, INT_VAL);
                assertEquals(INT_VAL, memory.readInt(object, addr + i),
                        "on-heap ordered int write should be visible to read at offset " + i);
            }
    }
        ...
        for (int i = 0; i <= 64; i++)
            if (onHeap == null) {
                ...
                assertEquals(LONG_VAL, memory.readLong(addr + i),
                        "off-heap long read should return written value at offset " + i);
            } else {
                memory.writeLong(object, addr + i, LONG_VAL);
                assertEquals(LONG_VAL, memory.readLong(object, addr + i),
                        "on-heap long read should return written value at offset " + i);
            }
    }
        ...
        for (int i = 0; i <= 64; i++)
            if (onHeap == null) {
                ...
                assertEquals(FLOAT_VAL, memory.readFloat(addr + i), EPSILON,
                        "off-heap float read should return written value at offset " + i);
            } else {
                memory.writeFloat(object, addr + i, 1);
                assertEquals(FLOAT_VAL, memory.readFloat(object, addr + i), EPSILON,
                        "on-heap float read should return written value at offset " + i);
            }
    }
        ...
        for (int i = 0; i <= 64; i++)
            if (onHeap == null) {
                ...
                assertEquals(DOUBLE_VAL, memory.readDouble(addr + i), EPSILON,
                        "off-heap double read should return written value at offset " + i);
            } else {
                memory.writeDouble(object, addr + i, DOUBLE_VAL);
                assertEquals(DOUBLE_VAL, memory.readDouble(object, addr + i), EPSILON,
                        "on-heap double read should return written value at offset " + i);
            }
    }
    ...
    void writeOrderedLong() {
        for (int i = (int) (-addr & 7); i <= 64; i += 8)
            if (onHeap == null) {
                ...
                assertEquals(LONG_VAL, memory.readLong(addr + i),
                        "off-heap ordered long write should be visible to read at offset " + i);
            } else {
                memory.writeOrderedLong(object, addr + i, LONG_VAL);
                assertEquals(LONG_VAL, memory.readLong(object, addr + i),
                        "on-heap ordered long write should be visible to read at offset " + i);
            }
        ...
        for (int i = 0; i <= 64; i += 4)
            try {
                if (onHeap == null) {
                    ...
                    assertTrue(actual,
                            "off-heap compareAndSwapInt should succeed with matching expected value at offset " + i);
                    assertEquals(INT_VAL, memory.readInt(addr + i),
                            "off-heap CAS int should update value when expected matches at offset " + i);
                } else {
                    memory.writeInt(object, addr + i, 0);
                    final boolean actual = memory.compareAndSwapInt(object, addr + i, 0, INT_VAL);
                    assertTrue(actual,
                            "on-heap compareAndSwapInt should succeed with matching expected value at offset " + i);
                    assertEquals(INT_VAL, memory.readInt(object, addr + i),
                            "on-heap CAS int should update value when expected matches at offset " + i);
                }
        ...
        for (int i = (int) (-addr & 7); i <= 64; i += 8)
            try {
                if (onHeap == null) {
                    ...
                    assertTrue(actual,
                            "off-heap compareAndSwapLong should succeed with matching expected value at offset " + i);
                    assertEquals(LONG_VAL, memory.readLong(addr + i),
                            "off-heap CAS long should update value when expected matches at offset " + i);
                } else {
                    memory.writeLong(object, addr + i, 0);
                    final boolean actual = memory.compareAndSwapLong(object, addr + i, 0, LONG_VAL);
                    assertTrue(actual,
                            "on-heap compareAndSwapLong should succeed with matching expected value at offset " + i);
                    assertEquals(LONG_VAL, memory.readLong(object, addr + i),
                            "on-heap CAS long should update value when expected matches at offset " + i);
                }
            } catch (MisAlignedAssertionError e) {
            ...
            try {
                if (onHeap == null) {
                    ...
                    assertEquals(initialValue, previous,
                            "off-heap getAndSetInt should return previous value at offset " + i);
                    assertEquals(INT_VAL, memory.readInt(addr + i),
                            "off-heap getAndSetInt should update memory to new value at offset " + i);
                } else {
                    memory.writeInt(object, addr + i, initialValue);
                    final int previous = memory.getAndSetInt(object, addr + i, INT_VAL);
                    assertEquals(initialValue, previous,
                            "on-heap getAndSetInt should return previous value at offset " + i);
                    assertEquals(INT_VAL, memory.readInt(object, addr + i),
                            "on-heap getAndSetInt should update memory to new value at offset " + i);
                }
            } catch (MisAlignedAssertionError e) {
                if (memory.safeAlignedInt(addr + i))
                    throw e;
    ...
    void readVolatileByte() {
        for (int i = 0; i <= 64; i++)
            if (onHeap == null) {
                ...
                assertEquals(BYTE_VAL, actual, "off-heap readVolatileByte should return written value at offset " + i);
            } else {
                memory.writeByte(object, addr + i, BYTE_VAL);
                final byte actual = memory.readVolatileByte(object, addr + i);
                assertEquals(BYTE_VAL, actual, "on-heap readVolatileByte should return written value at offset " + i);
            }
    }
    ...
    void readVolatileShort() {
        for (int i = 0; i <= 64; i += 2)
            if (onHeap == null) {
                ...
                assertEquals(SHORT_VAL, actual, "off-heap readVolatileShort should return written value at offset " + i);
            } else {
                memory.writeShort(object, addr + i, SHORT_VAL);
                final short actual = memory.readVolatileShort(object, addr + i);
                assertEquals(SHORT_VAL, actual, "on-heap readVolatileShort should return written value at offset " + i);
            }
    }
    ...
    void readVolatileInt() {
        for (int i = 0; i <= 64; i += 4)
            if (onHeap == null) {
                ...
                assertEquals(INT_VAL, actual, "off-heap readVolatileInt should return written value at offset " + i);
            } else {
                memory.writeInt(object, addr + i, INT_VAL);
                final int actual = memory.readVolatileInt(object, addr + i);
                assertEquals(INT_VAL, actual, "on-heap readVolatileInt should return written value at offset " + i);
            }
    }
        ...
        for (int i = 0; i <= 64; i += 4)
            if (onHeap == null) {
                ...
                assertEquals(FLOAT_VAL, actual, EPSILON,
                        "off-heap readVolatileFloat should return written value at offset " + i);
            } else {
                memory.writeFloat(object, addr + i, FLOAT_VAL);
                final float actual = memory.readVolatileFloat(object, addr + i);
                assertEquals(FLOAT_VAL, actual, EPSILON,
                        "on-heap readVolatileFloat should return written value at offset " + i);
            }
    }
    ...
    void readVolatileLong() {
        for (int i = (int) (-addr & 7); i <= 64; i += 8)
            if (onHeap == null) {
                ...
                assertEquals(LONG_VAL, actual, "off-heap readVolatileLong should return written value at offset " + i);
            } else {
                memory.writeLong(object, addr + i, LONG_VAL);
                final long actual = memory.readVolatileLong(object, addr + i);
                assertEquals(LONG_VAL, actual, "on-heap readVolatileLong should return written value at offset " + i);
            }
    }
        ...
        for (int i = (int) (-addr & 7); i <= 64; i += 8)
            if (onHeap == null) {
                ...
                assertEquals(DOUBLE_VAL, actual, EPSILON,
                        "off-heap readVolatileDouble should return written value at offset " + i);
            } else {
                memory.writeDouble(object, addr + i, DOUBLE_VAL);
                final double actual = memory.readVolatileDouble(object, addr + i);
                assertEquals(DOUBLE_VAL, actual, EPSILON,
                        "on-heap readVolatileDouble should return written value at offset " + i);
            }
    }
        ...
        for (int i = 0; i <= 64; i++)
            if (onHeap == null) {
                ...
                assertEquals(BYTE_VAL, memory.readByte(addr + i),
                        "off-heap writeVolatileByte should be visible to read at offset " + i);
            } else {
                memory.writeVolatileByte(object, addr + i, BYTE_VAL);
                assertEquals(BYTE_VAL, memory.readByte(object, addr + i),
                        "on-heap writeVolatileByte should be visible to read at offset " + i);
            }
    }
        ...
        for (int i = 0; i <= 64; i += 2)
            if (onHeap == null) {
                ...
                assertEquals(SHORT_VAL, memory.readShort(addr + i),
                        "off-heap writeVolatileShort should be visible to read at offset " + i);
            } else {
                memory.writeVolatileShort(object, addr + i, SHORT_VAL);
                assertEquals(SHORT_VAL, memory.readShort(object, addr + i),
                        "on-heap writeVolatileShort should be visible to read at offset " + i);
            }
    }
    ...
    void writeVolatileInt() {
        for (int i = 0; i <= 64; i += 4)
            if (onHeap == null) {
                ...
                assertEquals(INT_VAL, memory.readInt(addr + i),
                        "off-heap writeVolatileInt should be visible to read at offset " + i);
            } else {
                memory.writeVolatileInt(object, addr + i, INT_VAL);
                assertEquals(INT_VAL, memory.readInt(object, addr + i),
                        "on-heap writeVolatileInt should be visible to read at offset " + i);
            }
    }
    ...
    void writeVolatileFloat() {
        for (int i = 0; i <= 64; i += 4)
            if (onHeap == null) {
                ...
                assertEquals(FLOAT_VAL, memory.readFloat(addr + i), EPSILON,
                        "off-heap writeVolatileFloat should be visible to read at offset " + i);
            } else {
                memory.writeVolatileFloat(object, addr + i, FLOAT_VAL);
                assertEquals(FLOAT_VAL, memory.readFloat(object, addr + i), EPSILON,
                        "on-heap writeVolatileFloat should be visible to read at offset " + i);
            }
    }
    ...
    void writeVolatileLong() {
        for (int i = (int) (-addr & 7); i <= 64; i += 8)
            if (onHeap == null) {
                ...
                assertEquals(LONG_VAL, memory.readLong(addr + i),
                        "off-heap writeVolatileLong should be visible to read at offset " + i);
            } else {
                memory.writeVolatileLong(object, addr + i, LONG_VAL);
                assertEquals(LONG_VAL, memory.readLong(object, addr + i),
                        "on-heap writeVolatileLong should be visible to read at offset " + i);
            }
    }
    ...
    void writeVolatileDouble() {
        for (int i = (int) (-addr & 7); i <= 64; i += 8)
            if (onHeap == null) {
                ...
                assertEquals(DOUBLE_VAL, memory.readDouble(addr + i), EPSILON,
                        "off-heap writeVolatileDouble should be visible to read at offset " + i);
            } else {
                memory.writeVolatileDouble(object, addr + i, DOUBLE_VAL);
                assertEquals(DOUBLE_VAL, memory.readDouble(object, addr + i), EPSILON,
                        "on-heap writeVolatileDouble should be visible to read at offset " + i);
            }
    }
        ...
        for (int i = 0; i <= 64; i += 4)
            try {
                if (onHeap == null) {
                    ...
                    assertEquals(INT_VAL, actual, "off-heap addInt should return updated value at offset " + i);
                    assertEquals(INT_VAL, memory.readInt(addr + i),
                            "off-heap addInt should update memory to new value at offset " + i);
                } else {
                    memory.writeInt(object, addr + i, 0);
                    final int actual = memory.addInt(object, addr + i, INT_VAL);
                    assertEquals(INT_VAL, actual, "on-heap addInt should return updated value at offset " + i);
                    assertEquals(INT_VAL, memory.readInt(object, addr + i),
                            "on-heap addInt should update memory to new value at offset " + i);
                }
            } catch (MisAlignedAssertionError e) {
                if (memory.safeAlignedInt(addr + i))
                    throw e;
        ...
        for (int i = (int) (-addr & 7); i <= 64; i += 8)
            try {
                if (onHeap == null) {
                    ...
                    assertEquals(LONG_VAL, actual, "off-heap addLong should return updated value at offset " + i);
                    assertEquals(LONG_VAL, memory.readLong(addr + i),
                            "off-heap addLong should update memory to new value at offset " + i);
                } else {
                    memory.writeLong(object, addr + i, 0);
                    final long actual = memory.addLong(object, addr + i, LONG_VAL);
                    assertEquals(LONG_VAL, actual, "on-heap addLong should return updated value at offset " + i);
                    assertEquals(LONG_VAL, memory.readLong(object, addr + i),
                            "on-heap addLong should update memory to new value at offset " + i);
                }
            } catch (MisAlignedAssertionError e) {
                if (memory.safeAlignedLong(addr + i))
                    throw e;
