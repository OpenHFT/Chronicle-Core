...
| `background.releaser.thread` | `true` | Starts the BACKGROUND_RESOURCE_RELEASER thread if `background.releaser` is also `true`. Otherwise queued resources must be released manually. See link:https://github.com/OpenHFT/Chronicle-Core/blob/ea/README.adoc#releasing-resources[README] for details. | `BG_RELEASER_THREAD` (boolean)
