...
| disable.perf.info | `false` | if 'true', returns NullExceptionHandler | disablePerfInfo (boolean)
| disable.resource.warning | `false` | If 'true', returns that resource tracing is turned on | boolean
| disable.thread.safety | `false` | If 'true', threadSafetyCheck throws IllegalStateException in AbstractReferenceCounted.java and message thrown stating which component is not thread safe | _DISABLE_THREAD_SAFETY_ (boolean)
...
| jvm.safepoint.enabled | `false` | If 'true', inserts a low-cost Java safe-point, which can help to find blockages. Jvm.safepoint can also be added when monitoring the event loop in link:https://github.com/OpenHFT/Chronicle-Threads#monitoring-the-event-loop[Chronicle-Threads] to help identify hotspots in the code | SAFEPOINT_ENABLED (boolean)
...
| strict.discard.warning | `false` | If 'true', message is displayed stating that resources have been 'discarded without being released by' + referencesAsString() | _STRICT_DISCARD_WARNING_ (boolean)
